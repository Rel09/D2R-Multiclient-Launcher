...
void        Settings::SaveConfig() {
    ...
    if (RegCreateKeyExA(HKEY_CURRENT_USER, REGISTRY_PATH, 0, NULL, 0, KEY_WRITE, NULL, &hKey, NULL) != ERROR_SUCCESS) {
        MessageBoxA(0, "Error: Couldn't save Registry key.\nThis Process uses the Registry 'HKEY_CURRENT_USER\\SOFTWARE\\D2R_MULTI'", "D2RMulti", 0);
        return;
    }
...
void        Settings::ExportConfigToCSV() {
    ...
    if (!file.is_open()) {
        MessageBoxA(0, ("Failed to open file for writing: " + filename).c_str(), "D2RMulti", 0);
    ...
    MessageBoxA(0, ("Configuration exported to: " + filename).c_str(), "D2RMulti", 0);
}
void        Settings::ImportConfigFromCSV() {
    std::string filename = GetExeDirectory() + "config.csv";
    std::ifstream file(filename);
    if (!file.is_open()) {
        MessageBoxA(0, ("Failed to open file for reading: " + filename).c_str(), "D2RMulti", 0);
    ...
    MessageBoxA(0, "Configuration imported successfully", "D2RMulti", 0);
}
